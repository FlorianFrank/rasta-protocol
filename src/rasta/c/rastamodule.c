 ...
 */
void
allocateBytes(struct RastaByteArray *result, struct RastaPacket *packet, rasta_hashing_context_t *hashing_context) {
...
void rastaModuleToBytes(struct RastaByteArray *result, struct RastaPacket *packet,
                        rasta_hashing_context_t *hashing_context) {
    unsigned char temp[4];
    allocateBytes(result, packet, hashing_context);

    if (rastamodule_lasterror != RASTA_ERRORS_NONE) return;
...
struct RastaByteArray rastaRedundancyPacketToBytes(struct RastaRedundancyPacket packet, rasta_hashing_context_t * hashing_context){
    ...
    rastaModuleToBytes(&internal_packet_bytes, &packet.data, hashing_context);
...
void bytesToRastaRedundancyPacket(struct RastaRedundancyPacket *packet, struct RastaByteArray *data,
    ...
    // convert to rasta packet
     bytesToRastaPacket(&packet->data, &internal_packet_bytes, hashing_context);
